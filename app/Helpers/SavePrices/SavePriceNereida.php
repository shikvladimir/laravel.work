    ...
    public function pullPriceNereida()
    {
        ...
        if (!$connection) {
            ...
            if (!empty($message_id)) {
                ...
                if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/../prices_for_processing/price_nereida/Nereida_Price.xlsx')) {
                    foreach (glob($_SERVER['DOCUMENT_ROOT'] . '/../prices_for_processing/price_nereida/Nereida_Price.xlsx') as $file) {
                        unlink($file);
                    }
                }
                ...
                if (isset($structure->parts) && count($structure->parts)) {
                    for ($i = 1; $i < count($structure->parts); $i++) {
                        ...
                        $utfName = imap_utf8($attachments[$i]['name']);
                        $newName = explode('.', $utfName);
                        $newName[0] = 'Nereida_Price';
                        $newName = $newName[0] . '.' . $newName[1];

                        file_put_contents(
                            $_SERVER['DOCUMENT_ROOT'] . "/../prices_for_processing/price_nereida/" . $newName,
                ...
                dump('В течении последних 3 дней прайс от Nereida не приходил');
            }
